...
function buildAffineTransform (shearX, shearY, scaleX, scaleY, rot) {
  ...
  const tShear = [1 + shearX * shearY, shearX, shearY, 1, 0, 0]
  const tScale = [scaleX, 0, 0, scaleY, 0, 0]
  const u = V2.from(1, 0).rot(rot).transform(tScale).transform(tShear)
  const v = V2.from(0, 1).rot(rot).transform(tScale).transform(tShear)
...
function buildRotTransform (rot) {
  ...
  const u = V2.from(1, 0).rot(rot)
  const v = V2.from(0, 1).rot(rot)
