          ...
          if (GRAVITY_PROPS.currentPointIndex === 0) {
            ...
            if (CONTROLS.SPEED_UP.state === 1) {
              MOVE_STATE.speed = Math.min(
                USER_PARAMS.MAX_SPEED,
                MOVE_STATE.speed + USER_PARAMS.ACCELERATION,
              );
            }
            if (CONTROLS.SPEED_DOWN.state === 1) {
              MOVE_STATE.speed = Math.max(
                USER_PARAMS.MIN_SPEED,
                MOVE_STATE.speed - USER_PARAMS.DECELERATION,
              );
            }
            ...
            if (CONTROLS.ROTATE_RIGHT.state === 1) {
              MOVE_STATE.offset_rotation = Math.min(
                USER_PARAMS.MAX_ROTATION * ONE_DEGREE,
                MOVE_STATE.offset_rotation + USER_PARAMS.ROTATION_CHANGE * ONE_DEGREE,
              );
            }
            if (CONTROLS.ROTATE_LEFT.state === 1) {
              MOVE_STATE.offset_rotation = Math.max(
                USER_PARAMS.MIN_ROTATION * ONE_DEGREE,
                MOVE_STATE.offset_rotation - USER_PARAMS.ROTATION_CHANGE * ONE_DEGREE,
              );
            }
            if (CONTROLS.ZOOM_IN.state === 1) {
              MOVE_STATE.zoom = Math.min(
                USER_PARAMS.MAX_ZOOM,
                MOVE_STATE.zoom + USER_PARAMS.ZOOM_CHANGE,
              );
            }
            if (CONTROLS.ZOOM_OUT.state === 1) {
              MOVE_STATE.zoom = Math.max(
                USER_PARAMS.MIN_ZOOM,
                MOVE_STATE.zoom - USER_PARAMS.ZOOM_CHANGE,
              );
            }
          }
...
function main() {
  ...
  class CarModComponent extends React.Component {
    ...
    renderColorPicker(key, title, props) {
      ...
      return c("div", null, title, c("input", { type: "color", ...props }));
    }
    ...
    renderCheckbox(key, title, props) {
      ...
      return c("div", null, title, c("input", { type: "checkbox", ...props }));
    }
      ...
      return c(
        ...
        this.state.ENABLED
          && c(
            ...
            this.renderSlider("INIT_ZOOM", "Initial Zoom: ", {
              min: this.state.MIN_ZOOM,
              max: this.state.MAX_ZOOM,
              step: this.state.ZOOM_CHANGE,
            }),
            this.renderSlider("MIN_ZOOM", "Min Zoom: ", {
              min: -100,
              max: this.state.MAX_ZOOM,
              step: this.state.ZOOM_CHANGE,
            }),
            this.renderSlider("MAX_ZOOM", "Max Zoom: ", {
              min: this.state.MIN_ZOOM,
              max: 100,
              step: this.state.ZOOM_CHANGE,
            }),
            this.renderSlider("ZOOM_CHANGE", "Zoom Change: ", { min: -1, max: 1, step: 0.01 }),
            this.renderSlider("INIT_SPEED", "Initial Speed: ", {
              min: this.state.MIN_SPEED,
            ...
            this.renderSlider("ACCELERATION", "Acceleration: ", {
              min: this.state.MIN_SPEED,
              max: this.state.MAX_SPEED,
              step: 0.01,
            }),
            this.renderSlider("DECELERATION", "Deceleration: ", {
              min: this.state.MIN_SPEED,
              max: this.state.MAX_SPEED,
              step: 0.01,
            }),
            ...
            this.renderSlider("INIT_ROTATION", "Initial Rotation: ", {
              min: this.state.MIN_ROTATION,
              max: this.state.MAX_ROTATION,
              step: this.state.ROTATION_CHANGE,
            }),
