...
// @downloadURL  https://github.com/Malizma333/linerider-userscript-mods/raw/master/mods/line-rider-xavi-animation-helper.user.js
// @updateURL    https://github.com/Malizma333/linerider-userscript-mods/raw/master/mods/line-rider-xavi-animation-helper.user.js
...
class AnimateMod {
  ...
  onUpdate(nextState = this.state) {
    ...
    if (
      !this.active()
      || !(this.state.groupBegin <= this.state.layerOrigin && this.state.layerOrigin <= this.state.groupEnd)
    ) {
      this.componentUpdateResolved = true;
      return;
    }
    ...
    for (let i = 0; i < this.state.aLength - 1; i++) {
      ...
      for (let line of selectedLines) {
        ...
        transformedLines.push({
          ...
          layer: layersArray[layerIndex].id,
          id: null,
          x1: p1.x,
          y1: p1.y,
          x2: p2.x,
    ...
    render() {
      ...
      if (this.state.active) {
        tools = [
          ...
          this.renderSlider(
            "groupBegin",
            { min: 0, max: this.state.numLayers - 1, step: 1 },
            "Group Begin (Inclusive)",
          ),
          ...
          this.renderSlider("aLength", { min: 1, max: 1000, step: 1 }, "Animation Length"),
        ];
